...
function toggleGlobal(checkbox, selected, columns) {
  ...
  document.querySelectorAll("div.table-tabs").forEach((t) => {
    ...
    var visible = 0;
    t.parentElement.querySelectorAll("div." + id).forEach(function (elem) {
      if (
        selectAll ||
...
document.addEventListener("DOMContentLoaded", function (e) {
  ...
  if (subnav && subnav.lastElementChild) {
    subnav.lastElementChild.scrollIntoView({
      behavior: "instant",
      inline: "start",
      block: "nearest",
    });
  ...
  document.querySelectorAll("input.filter-input").forEach(function (input) {
    ...
    input.addEventListener("input", function (e) {
      ...
      input.parentNode.parentNode
        .querySelectorAll("ol.toc-list li")
        .forEach((li) => {
      ...
      expandedHeight = Math.min(
        ...
        Math.max(
          ...
          toc.querySelector("div.toc-header").offsetHeight +
            toc.querySelector("ol.toc-list").scrollHeight +
            10
        )
      );
  ...
  document.querySelectorAll("h1, h2, h3, h4, h5, h6").forEach((hdr, idx) => {
  ...
  function initSectionData() {
    sections = [{ id: "", top: 0 }].concat(
      Array.from(
        main.querySelectorAll("section[id], h2[id], h2 a[id], div[id]")
      )
        .filter((e) => {
          return (
            sidebar.querySelector(
              'a[href="#' + encodeURI(e.getAttribute("id")) + '"]'
            ) !== null
          );
        })
        .map((e) => {
  ...
  function handleScroll() {
    ...
    if (scrollHeight - scrollTop < window.innerHeight + 10) {
      ...
      for (var i = 0; i < sections.length; i++) {
        ...
        if (
          top + (bottom - top) / 2 > scrollTop ||
          bottom > scrollTop + window.innerHeight / 3
        ) {
  ...
  function setSelected(hash) {
    ...
    if (prev) prev.classList.remove("current-selection");
    prevHash = hash;
    if (hash) {
  ...
  if (sidebar) {
    ...
    sidebar
      .querySelector("button.hide-sidebar")
      .addEventListener("click", () => {
        sidebar.classList.add("hide-sidebar");
        sessionStorage.setItem("sidebar", "hidden");
      });
    sidebar
      .querySelector("button.show-sidebar")
      .addEventListener("click", () => {
